...
- **收件箱处理**：扫描、分类、成本入库、归档
- **日志分析**：通过 `[[项目名]]` 链接 + AI 推断识别项目归属，自动更新标签（待整理→已整理）
- **项目文件管理**：基于模板创建、增量更新时间线和任务状态
- **系统控制台维护**：更新 `_dashboard.md` 的全局状态，每条附日志链接可跳转
...
→ 标记已入库 → 移至 50_收件箱备份
       ↓
分析当日日志 (Step 7) → [[链接]]/AI推断 → 识别项目/任务/日常
       ↓
...
- Obsidian vault 路径：`D:\iCloudDrive\iCloud~md~obsidian\QZ_Vault\`
...
| 版本 | 日期 | 说明 |
|------|------|------|
| V2.2 | 2026-04-22 | 日志按年月子文件夹归档；title加⏳/✅尾缀标识整理状态；创建日志时自动建子文件夹 |
| V2.1 | 2026-04-21 | 项目识别改为[[链接]]+AI推断双层机制；Dashboard条目附日志链接；日志标签待整理/已整理 |
